  ...
  module V1
    class PortfoliosController < ApiController
      ...
      def regenerate_fitgap
        portfolio  = tenant_portfolio(params[:id])
        vacancy_id = params[:vacancy_id]
      ...
      # Portfolio carries no tenant_id of its own; only Session does, through
      # TenantScoped. A bare Portfolio.find therefore reached across customer
      # boundaries by id alone, the same defect already fixed on the override
      # endpoint. Every lookup goes through sessions and says so explicitly.
      def tenant_portfolio(id)
        Portfolio.joins(:session).where(sessions: { tenant_id: current_tenant_id }).find(id)
      end
